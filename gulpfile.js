...
const karmaServer = require('experience-karma').karma.Server;

const karma = (singleRun, done) => new karmaServer({
    configFile: __dirname + '/karma.conf.js',
    singleRun
}, err=>done(err ? new Error(err) : null)).start();


gulp.task('test', ['dist'], done => karma(true, done));
gulp.task('karma', ['dist'], done => karma(false, done));